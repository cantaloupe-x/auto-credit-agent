...
try:
    from .domain import RULE_MODEL_VERSION
except ImportError:
    from domain import RULE_MODEL_VERSION
...
def assess(app):
    ...
    reasons = []
    if income <= 0:
        return {"score": 0, "level": "高风险", "recommendation": "补充收入资料", "reasons": ["收入缺失或无效"], "model_version": RULE_MODEL_VERSION}
    dti = debt / income
    ...
    if "loan_amount" in app:
        vehicle_price = float(app.get("vehicle_price", 0))
        financed_total = float(app.get("down_payment", 0)) + float(app.get("loan_amount", 0))
        tolerance = max(vehicle_price * .01, 1000)
        if abs(financed_total - vehicle_price) > tolerance:
            score -= 10; reasons.append("首付金额与贷款金额之和同车辆价格不一致，需人工核验")
    if int(app.get("loan_term", 36)) > 60:
        reasons.append("贷款期限超过 60 个月，需人工核验长期偿付安排")
    ...
    return {"score": score, "level": level, "recommendation": recommendation, "reasons": reasons, "model_version": RULE_MODEL_VERSION}
