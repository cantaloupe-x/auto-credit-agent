...
def assess(app):
    ...
    # 以下两条来自 PR #1：金额一致性与超长期限提示。
    if "loan_amount" in app:
        financed_total = down_payment + float(app.get("loan_amount", 0))
        tolerance = max(vehicle_price * 0.01, 1000)
        if abs(financed_total - vehicle_price) > tolerance:
            score -= 10
            reasons.append("首付金额与贷款金额之和同车辆价格不一致，需人工核验")
