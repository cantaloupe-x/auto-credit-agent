...
镜像用仓库根目录的 `Dockerfile`，Render 的配置见 `render.yaml`。

数据库路径由环境变量 `AUTO_CREDIT_DB` 控制，部署时指向持久磁盘的挂载目录（例如 `/var/data/auto_credit.db`）；
不设这个变量就用仓库里的 `data/auto_credit.db`，本地开发不受影响。监听端口读平台注入的 `PORT`。

注意：免费套餐的文件系统通常是临时的，重启或重新部署会丢数据。要保住「重启后案件还在」这条验收标准，
需要挂一块持久磁盘（`render.yaml` 里已经配好 1GB），否则演示时只能看到每次启动重新导入的两条初始案件。
